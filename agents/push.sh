...
if ! git diff --quiet || ! git diff --cached --quiet; then
  echo "Error: you have uncommitted changes. Commit or stash before pushing." >&2
  exit 1
fi
...
echo "Pushing subtree '${AGENTS_PREFIX}' to ${AGENTS_REMOTE}/${AGENTS_BRANCH}..."
git subtree push --prefix="${AGENTS_PREFIX}" "${AGENTS_REMOTE}" "${AGENTS_BRANCH}"
